...
void Loader(ifstream& input_file, int* PC, int* MEM_usage)
{
    int entry, size = 0;
    struct instruction temp;
    bool first = true, flag = false;
    while (input_file >> entry) size++;
    *MEM_usage = size;
    machine_code = new int[size];
    input_file.clear();
    input_file.seekg(0);
    size = 0;
...
int ID_part1(struct instruction* current_inst, int* Reg_read)
{
    ...
    else if (current_inst->opcode < 5)
    {
        *Reg_read = 2 + *Reg_read;
        ...
        strcpy_s(current_inst->mnemonic, inst[current_inst->opcode]);
        help = 0x0000000f;
...
int main(int argc, char** argv) {
    ...
    ifstream machp(argv[1], ios::_Nocreate);
    if (!machp) {
